...
public class Conversion {
	...
	public static int iHitIdx = -1;
	public static int[] iImageMove = {-1, -1, -1, -1};
	
	public static Point getPositionByRate(int OriX, int OriY, int HitIdx, int OriWidth, int Rate) {
		Point result = null;
		
		int iHitX = HitIdx % (OriWidth / Rate);
		int iHitY = HitIdx / (OriWidth / Rate);
		int iRealX = OriX + (iHitX * Rate);
		int iRealY = OriY + (iHitY * Rate);
		
		result = new Point(iRealX, iRealY);
		
		return result;
	}
	
	public static int[] ConvImageMove(int[] img1, int[] img2, int iWidth) {
		int[] iCheck = {-1, -1, -1, -1};
		int a = 0;
		int b = 0;
		int x = 0;
	...
	public static double [] ConvMAD(int [] image, int [] check, int ImageWidth, int CheckWidth) {
		...
		for(int i = 0; i<image.length;i++) {
			...
			for(int j = 0; j<check.length;j++) {
				...
				iTempImage = image[((ImageRow + CheckRow) * ImageWidth) + (ImageCol + CheckCol)];
				iTempCheck = check[(CheckRow * CheckWidth) + CheckCol];
				dTempResult = dTempResult + Math.abs(iTempImage - iTempCheck);
			}
			result[(ImageRow * Width) + ImageCol] = dTempResult / check.length;
		}
